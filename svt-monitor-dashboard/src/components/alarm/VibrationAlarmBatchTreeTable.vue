<template>
  <el-table
    class="vibration-alarm-batch-tree-table"
    :data="rows"
    row-key="id"
    border
    height="100%"
  ...
  >
    ...
    <el-table-column label="报警轴向" min-width="120">
      <template #default="{ row }">
...
const { getPointName, getReceiverName } = useAlarmBatchRowLabels({ parseDataJson: false })

const isSelectableRow = (row: AlarmBatchTreeRow) => row.rowType === 'event'

const rowClassName = ({ row }: { row: AlarmBatchTreeRow }) =>
  row.rowType === 'equipment' ? 'vibration-alarm-tree-row--equipment' : 'vibration-alarm-tree-row--event'
...
:deep(.vibration-alarm-tree-row--equipment) {
  font-weight: 600;
}
</style>
