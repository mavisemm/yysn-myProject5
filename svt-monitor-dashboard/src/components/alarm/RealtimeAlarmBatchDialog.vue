<template>
  <el-dialog v-model="store.realtimeAlarmVisible" title="实时报警批量操作" width="1100px" align-center
    class="alarm-batch-dialog" @close="store.closeRealtimeAlarm">
...
const getDisabledStartMinutes = (hour: number) => getDisabledMinutesByRaw(store.realtimeAlarmQuery.startTime, hour)
...
const getDisabledEndMinutes = (hour: number) => getDisabledMinutesByRaw(store.realtimeAlarmQuery.endTime, hour)
