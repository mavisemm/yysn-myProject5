...
const props = withDefaults(
  defineProps<{
    ...
    autoYAxisSamplingThreshold?: number
...
const scheduleOptionRefresh = () => {
  ...
  if (optionUpdateTimer != null) {
    window.clearTimeout(optionUpdateTimer)
    optionUpdateTimer = null
  }
  if (optionUpdateRaf != null) {
    window.cancelAnimationFrame(optionUpdateRaf)
  ...
  optionUpdateTimer = window.setTimeout(run, delay)
}
...
watch(
  () => [
    ...
    props.autoYAxisOnZoomDebounceMs,
    props.autoYAxisOnZoomPaddingRatio,
    props.rangeControlsXAxisIndex,
  ],
  () => {
    if (chartInstance.value) {
      attachAutoYAxisListener()
